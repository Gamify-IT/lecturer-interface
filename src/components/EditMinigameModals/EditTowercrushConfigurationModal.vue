...
import {
  getTowercrushConfig,
  postTowercrushConfig,
} from "@/ts/rest-clients/towercrush-rest-client";
...
let configuration = ref(new TowercrushConfiguration([]));
const question = ref();
...
function resetModal() {
  if (minigame.value.configurationId != undefined) {
    getTowercrushConfig(minigame.value.configurationId)
      .then((response) => {
...
function handleOk() {
  postTowercrushConfig(configuration.value)
    .then((response) => {
...
function removeQuestion(text: string) {
  let filteredQuestions: ITowercrushQuestion[] = [];
...
function downloadConfiguration() {
  ...
  const clonedQuestions = Array<ITowercrushQuestion>();
  ...
  saveAs(blob, "towercrush-configuration.json");
}
...
<template>
  <b-modal
    title="Edit Towercrush configuration"
    id="edit-modal"
    v-model="showModal"
    @hidden="hiddenModal"
    @ok="handleOk"
  ...
  <b-modal
    id="add-question-towercrush"
    title="Add Question to Towercrush configuration"
